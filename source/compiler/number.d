...
class Number
{
    ...
    public static string floatToHex(float value, string prefix = "")
    {
        ...
        return format("%s%02x,%s%02x,%s%02x,%s%02x", prefix, exponent, prefix,
                binaryMant[0], prefix, binaryMant[1], prefix, binaryMant[2]);
    }
    ...
    public static string integralToHex(int value, Type type, bool validate = false,
            string prefix = "")
    {
        if (validate)
        {
        ...
        {
            bytes ~= prefix ~ to!string(
                    rightJustifier(to!string((value & (255 << (i * 8))) >> (i * 8), 16), 2, '0'));
        }
        return bytes.join(",");
    }
}
