...
class Factor : AbstractExpression
{
    ...
    {
        ParseTree child = this.node.children.length == 1
            ? this.node.children[0] : this.node.children[1];
        switch (child.name)
        {
        case "XCBASIC.Number":
            Number num = new Number(child, compiler);
            return num.type == compiler.getTypes().get(Type.FLOAT)
                ? num.floatVal : cast(float) num.intVal;
    ...
    public void eval()
    {
        ...
        case "XCBASIC.Address":
            ...
            catch (Exception e)
            {
                ...
                {
                    this.asmCode = "    paddr " ~ this.compiler.getLabels()
        ...
        if (unOp.length > 0)
        {
            ...
            if (!this.type.isNumeric())
            {
                compiler.displayError(
                        "The " ~ unOp ~ " operator cannot be used with non-numeric types");
            }
            if (unOp == "-" && (this.type.name == Type.DEC
                    || this.type.name == Type.UINT8 || this.type.name == Type.UINT16))
            {
