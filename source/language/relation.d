...
class Relation : AbstractExpression
{
    ...
    public void eval()
    {
        ...
        if (this.node.children.length > 1)
        {
            ...
            if ((left.getType().name == Type.INT16 && right.getType()
                    .name == Type.UINT16) || (left.getType()
                    .name == Type.UINT16 && right.getType().name == Type.INT16))
            {
                expectedType = compiler.getTypes().get(Type.INT24);
            }
            // If one type is a string and another one is numeric then
            // we can't evaluate
        else if ((left.getType().name == Type.STRING && right.getType()
                    .name != Type.STRING) || (left.getType()
                    .name != Type.STRING && right.getType().name == Type.STRING))
            {
                compiler.displayError("Strings can't be compared to other types (trying to compare '" ~ left.getType()
                        .name ~ "' to '" ~ right.getType().name ~ "')");
            }
            ...
            {
                expectedType = left.getType().comparePrecedence(right.getType())
            ...
            if (left.getType().name == Type.STRING)
            {
                if (op != "=" && op != "<>")
                {
                    compiler.displayError(
                            "Relational operator '" ~ op ~ "' not supported on strings");
                }
            }
