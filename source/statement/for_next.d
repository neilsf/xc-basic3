...
class For_stmt : Statement
{
    ...
    public void process()
    {
        ...
        if (access.getVariable() is null)
        {
            ...
            {
                compiler.displayNotice(
                        "Variable " ~ counterVar.name
                        ~ " auto-defined as STATIC because it is the counter of a FOR loop");
            }
        }
        else
        {
            ...
            if (counterVar.isDynamic)
            {
                compiler.displayError(
                        "Using a dynamic variable as counter in a FOR loop is not supported. Please define "
        ...
        if (counterVar.type.name == Type.STRING
                || counterVar.type.name == Type.DEC || !counterVar.type.isPrimitive)
        {
            compiler.displayError(
                    "Counter of a FOR loop can not be of type " ~ counterVar.type.name);
        }
        ...
        {
            Variable limitVar = Variable.create("FORLIM" ~ to!string(block.getId()),
        ...
        if (stepPresent)
        {
            ...
            stepVar = Variable.create("FORSTEP" ~ to!string(block.getId()),
                    counterVar.type, compiler, true);
        ...
        appendCode("    for" ~ counterVar.type.name ~ " " ~ blockId ~ ", " ~ counterVar.getAsmLabel()
                ~ ", " ~ limitValue ~ ", " ~ (stepPresent
                    ? stepVar.getAsmLabel() : "\"_void_\"") ~ ", " ~ (isLimitConstant
                    ? "1" : "0") ~ "\n");
    ...
    public void process()
    {
        ...
        if (node.children[0].children.length > 0)
        {
            ...
            try
            {
                ...
                {
                    compiler.displayError(
                            "Variable used in NEXT statement must match variable used in FOR statement");
                }
            }
            catch (Exception e)
            {
        ...
        appendCode("    next" ~ counterVar.type.name ~ " " ~ blockId ~ ", " ~ counterVar.getAsmLabel() ~ (stepVar is null
                ? ", \"_void_\"" : ", " ~ stepVar.getAsmLabel()) ~ "\n");
