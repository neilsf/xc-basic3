...
class Fun_stmt : Statement
{
    ...
    private void verifyName()
    {
        if (compiler.getRoutines().exists(this.name))
        {
            ...
            const Routine tmpRoutine = compiler.getRoutines().get(this.name,
                    this.routine.getArgsHash());
            if (tmpRoutine !is null)
            {
                ...
                if (!isDeclaration && tmpRoutine.isDefined)
                {
                    this.compiler.displayError(
                            "Routine \"" ~ this.routine.getNameWithArgTypes() ~ "\" already defined");
                }
                else if (isDeclaration)
                {
                    this.compiler.displayError(
                            "Routine \"" ~ this.routine.getNameWithArgTypes()
                            ~ "\" already declared");
                }
            ...
            else
            {
                ...
                {
                    this.compiler.displayError(
                            "Use the OVERLOAD keyword if you wish to overload routine \""
                            ~ this.name ~ "\"");
                }
            }
        }
    ...
    private void addRoutine()
    {
        ...
        if (!this.isInline)
        {
            ...
            {
                Variable argument = Variable.create(stub.name, stub.type,
    ...
    public void process()
    {
                    ...
                    if (procType.children.length > 1)
                    {
                        ...
                        if (len < 1 || len > stringMaxLength)
                        {
                            compiler.displayError(
                                    "String length must be between 1 and " ~ to!string(
                                    stringMaxLength));
                        }
        ...
        if (!isDeclaration)
        {
            ...
            {
                Routine tmpRoutine = compiler.getRoutines().get(this.name,
                        this.routine.getArgsHash());
