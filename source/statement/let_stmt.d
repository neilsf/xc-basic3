        ...
        if (access.getVariable() is null)
        {
            ...
            if (varNode.children.length > 1)
            {
                compiler.displayError(
                        "Cannot implicitly define variable with subscript and/or field access");
            }
            if (exp.getType().name == Type.STRING && !exp.isConstantString())
            {
                compiler.displayError(
                        "Cannot implicitly define variable of type STRING unless the right-hand side is a constant");
            }
            else if (!exp.getType().isPrimitive)
            {
                compiler.displayError(
                        "Only variables of primitive types can be implicitly defined. Use DIM to define \"" ~ join(
                        varNode.matches) ~ "\"");
            }
            ulong strLen = 0;
