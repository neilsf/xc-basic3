...
class Poke_stmt : Statement
{
    ...
    void process()
    {
        ...
        e2.setExpectedType(isDoke ? compiler.getTypes()
                .get(Type.UINT16) : compiler.getTypes().get(Type.UINT8));
        e2.eval();
        appendCode(e2.toString());
        if (e1.isConstant())
        {
            appendCode("    " ~ opCode ~ "_constaddr $" ~ to!string(to!int(e1.getConstVal()),
                    16) ~ "\n");
        }
        else
        {
