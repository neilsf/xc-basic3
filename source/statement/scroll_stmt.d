...
class Scroll_stmt : Statement
{
    ...
    void process()
    {
        ...
        {
            compiler.displayError("Wrong number of arguments, expected " ~ to!string(
                    expectedArgCount) ~ ", got " ~ to!string(args.children.length));
        }
        ulong ix = 0;
        string layer = "0";
        if (args.children.length == 2)
        {
            ...
            if (!e1.isConstant())
            {
                compiler.displayError("Argument #1 of " ~ toUpper(
                        node.matches[0]) ~ "SCROLL must be a constant 0 or 1");
            }
            ubyte v = to!ubyte(e1.getConstVal());
            if (v > 1 || v < 0)
            {
                compiler.displayError("Argument #1 of " ~ toUpper(
                        node.matches[0]) ~ "SCROLL must be between 0 or 1");
            }
            layer = to!string(v);
            ix++;
        }
