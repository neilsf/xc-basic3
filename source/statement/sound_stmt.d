    ...
    void process()
    {
        ...
        foreach (ref subCmd; voiceSubCmdNodes)
        {
            ...
            case "XCBASIC.VoiceSubCmdFilterOnOff":
                ...
                appendCode("    voice_" ~ toLower(node.matches.join()) ~ " " ~ to!string(
                        sidNo) ~ ", " ~ to!string(realVoiceNo) ~ "\n");
                break;

            case "XCBASIC.VoiceSubCmdADSR":
                if (node.children[0].children.length != 4)
                {
                    compiler.displayError("VOICE ADSR expects exactly 4 parameters, got " ~ to!string(
